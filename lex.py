...
# Token contains original text and the type of token
class Token:
    def __init__(self, tokenText, tokenKind):
        self.text = tokenText # text
...
class Lexer:
    def __init__(self, input):
        self.source = input + '\n' # Source code to lex as string. Added newline to simplify adding new token
    ...
    def getToken(self):
        ...
        if self.curChar == '+':
            ...
            if self.peek() == '=':
                ...
                self.abort("Expected !=, got only !" +  self.peek())
            ...
            while self.curChar != '\"':
                # no allow special character because printf will be used later
            ...
            text = self.source[startPos : self.curPos + 1] # + 1 becase there is a peek
            token = Token(text, TokenType.NUMBER)

        elif self.curChar.isalpha():
            # Leading character is an identifier
            startPos = self.curPos
